...
class CheckpointFunction_2(torch.autograd.Function):

    @staticmethod
    def forward(ctx, run_function, preserve_rng_state, *args):
        
        global previous_checkpoints, previous_checkpoint
        
        previous_checkpoints[ctx] = previous_checkpoint
        previous_checkpoint = ctx
        
        # print(f"forward {ctx}")
        # print(f"forward {id(ctx)}, {id(previous_checkpoints[ctx])}")
    ...
    @staticmethod
    def backward_forward(ctx):
        
        # print(f"backward_forward {id(ctx)}")
        ...
        return detached_inputs, outputs


    @staticmethod
    def backward_backward(ctx, detached_inputs, outputs, *args):

        # print(f"backward_backward {id(ctx)}")
    ...
    def backward(ctx, *args):
        global previous_checkpoints, previous_checkpoint, detached_inputs, outputs
        # print(f"backward {id(ctx)}, {id(previous_checkpoints[ctx])}")
        if detached_inputs is None:
            detached_inputs, outputs = CheckpointFunction_2.backward_forward(ctx)
            
        ret = None
        
        if previous_checkpoints[ctx]:
            ...
            with torch.cuda.stream(stream1):
                start1 = time.time()
                ret = CheckpointFunction_2.backward_backward(ctx, detached_inputs, outputs, *args)
                end1 = time.time()
                print(f"Execution time 1: {end1 - start1} seconds")
            with torch.cuda.stream(stream2):
                start2 = time.time()
                detached_inputs_new, outputs_new = CheckpointFunction_2.backward_forward(previous_checkpoints[ctx])
                end2 = time.time()
                print(f"Execution time 2: {end2 - start2} seconds")

            stream1.synchronize()
            stream2.synchronize()
            
            detached_inputs, outputs = detached_inputs_new, outputs_new
            ...
            previous_checkpoints = {}
            previous_checkpoint = None
            detached_inputs, outputs = None, None
            

        return ret


def checkpoint_2(function, *args, use_reentrant: bool = True, **kwargs):
    ...
    if use_reentrant:
        return CheckpointFunction_2.apply(function, preserve, *args)
    else:
